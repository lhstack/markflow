        ...
        let stream: StreamingResult<StreamingCompletionResponse> = Box::pin(stream! {
            ...
            while let Some(sse_result) = sse_stream.next().await {
                match sse_result {
                    ...
                    Ok(Event::Message(sse)) => {
                        saw_sse_message = true;
                        dump_anthropic_stream_event(
            ...
            if !saw_sse_message {
                yield Err(CompletionError::ProviderError(
                    "Anthropic SSE stream ended without any events".to_string(),
                ));
                return;
            }
...
fn dump_anthropic_stream_event(event: Option<&str>, id: Option<&str>, data: &str) {
    if std::env::var_os("RIG_DUMP_ANTHROPIC_STREAM").is_none()
        && std::env::var_os("RIG_DUMP_ANTHROPIC").is_none()
    {
        return;
    }
