    ...
    H: Clone + Default + Debug + WasmCompatSend + WasmCompatSync + 'static,
{
    ...
    fn create_request(
        ...
        {
            request.instructions = if !request.instructions.trim().is_empty() {
                let existing = request.instructions;
        ...
        if let Some(default_instructions) = &self.client.ext().default_instructions {
            request.instructions = merge_instructions(
                default_instructions,
                (!request.instructions.trim().is_empty()).then_some(request.instructions.as_str()),
            );
        }
