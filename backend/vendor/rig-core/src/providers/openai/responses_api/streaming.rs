        ...
        match item {
            ...
            Output::Message(message) => {
                if let Some(id) = message.id {
                    immediate.push(streaming::RawStreamingChoice::MessageId(id));
                }
            }
            Output::Unknown => {}
        }
...
fn raw_choices_from_json_event(
    data: &str,
    accumulator: &mut RawChoiceAccumulator,
    provider_name: &str,
    options: ResponsesStreamOptions,
    ...
    match value.get("type").and_then(serde_json::Value::as_str) {
        ...
        | Some("response.custom_tool_call_input.delta") => {
            ...
            if let (Some(item_id), Some(delta)) =
                (item_id, value.get("delta").and_then(serde_json::Value::as_str))
            {
                let internal_call_id = accumulator
                    .tool_call_internal_ids
...
fn dump_responses_stream_event(data: &str) {
    if std::env::var_os("RIG_DUMP_OPENAI_RESPONSES_STREAM").is_none()
        && std::env::var_os("RIG_DUMP_OPENAI_RESPONSES").is_none()
    {
        return;
    }

    tracing::info!("[openai_responses_stream] data: {data}");

    if let Ok(mut file) = std::fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open("rig_openai_responses_stream_raw.sse")
    {
        let _ = writeln!(file, "data: {data}\n");
    }
}
...
pub(crate) fn raw_choices_from_sse_body(
    ...
    for line in body.lines() {
        ...
        raw_choices.extend(raw_choices_from_json_event(
            data,
            &mut accumulator,
            provider_name,
            options,
...
fn message_id_from_response(response: &CompletionResponse) -> Option<String> {
    response.output.iter().find_map(|item| match item {
        Output::Message(message) => message.id.clone(),
        _ => None,
    })
}
        ...
        while let Some(event_result) = event_source.next().await {
            match event_result {
                ...
                Ok(Event::Message(evt)) => {
                    dump_responses_stream_event(&evt.data);
                    ...
                    let Ok(data) = data else {
                        ...
                        match raw_choices_from_json_event(
                            &evt.data,
                            &mut accumulator,
                            provider_name,
                            options,
    ...
    H: Clone + Default + std::fmt::Debug + WasmCompatSend + 'static,
{
    ...
    ) -> Result<streaming::StreamingCompletionResponse<StreamingCompletionResponse>, CompletionError>
    {
        ...
        let dump_id = std::time::SystemTime::now()
            ...
            .unwrap_or_default();
        if std::env::var_os("RIG_DUMP_OPENAI_RESPONSES_STREAM").is_some()
            || std::env::var_os("RIG_DUMP_OPENAI_RESPONSES").is_some()
        {
            let _ = std::fs::write(
                format!("rig_openai_responses_stream_request_{dump_id}.json"),
                serde_json::to_string_pretty(&request).unwrap_or_else(|_| String::new()),
            );
            let _ = std::fs::write("rig_openai_responses_stream_raw.sse", "");
        }
            ...
            || std::env::var_os("RIG_DUMP_OPENAI_RESPONSES").is_some()
        {
            ...
            tracing::info!("[openai_responses_stream_request] {request_json}");
            let _ = std::fs::write("rig_openai_responses_stream_request.json", request_json);
        }
