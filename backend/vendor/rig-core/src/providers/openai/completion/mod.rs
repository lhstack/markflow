...
pub enum Message {
    ...
    Assistant {
        ...
        // OpenAI-compatible providers expose hidden reasoning on non-standard
        // fields, and some require it to be echoed back on assistant tool-call
        // turns. DeepSeek/llama.cpp use `reasoning_content`; Sensenova uses
        // `reasoning`.
        #[serde(
...
impl ToolDefinition {
    ...
    pub fn with_strict(mut self) -> Self {
        self.function.strict = None;
        self
    }

    fn without_strict(mut self) -> Self {
        self.function.strict = None;
        self
    }
}
        ...
        match value {
            ...
            message::UserContent::Document(message::Document {
                ...
                DocumentSourceKind::String(_) => Err(message::MessageError::ConversionError(
                    "Documents must be base64-encoded for OpenAI file input".into(),
                )),
                DocumentSourceKind::FileId(_) => Err(message::MessageError::ConversionError(
            ...
            message::UserContent::Video(message::Video {
                ...
                DocumentSourceKind::Url(_) => Err(message::MessageError::ConversionError(
                    "URLs are not supported for video".into(),
                )),
                DocumentSourceKind::Raw(_) => Err(message::MessageError::ConversionError(
                    "Raw files are not supported for video".into(),
                )),
                DocumentSourceKind::FileId(_) => Err(message::MessageError::ConversionError(
                    "File IDs are not supported for video".into(),
                )),
                DocumentSourceKind::Unknown => Err(message::MessageError::ConversionError(
                    "Video has no body".into(),
                )),
                video => Err(message::MessageError::ConversionError(format!(
                    "Unsupported video type: {video:?}"
                ))),
            },
...
impl TryFrom<OpenAIRequestParams> for CompletionRequest {
    ...
    fn try_from(params: OpenAIRequestParams) -> Result<Self, Self::Error> {
        ...
        let tools: Vec<ToolDefinition> = tools
            .into_iter()
            .map(|tool| ToolDefinition::from(tool).without_strict())
            .collect();
    ...
    H: Clone + Default + std::fmt::Debug + WasmCompatSend + WasmCompatSync + 'static,
{
    ...
    async fn completion(
        ...
        if std::env::var_os("RIG_DUMP_OPENAI_CHAT").is_some()
            || std::env::var_os("RIG_DUMP_OPENAI_RESPONSES").is_some()
        {
            let request_json = serde_json::to_string_pretty(&request)?;
            tracing::info!("[openai_chat_request] {request_json}");
            let _ = std::fs::write("rig_openai_chat_request.json", request_json);
        }
