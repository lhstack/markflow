...
use crate::{
    agent_protocol::{default_agent_base_url, route_descriptions, route_enum_values},
    auth,
    db::Database,
    models::{AgentMcpServer, AgentMcpSettings, AgentProvider, UploadAsset},
};
use crate::mcp;
use crate::mcp_chat;
        ...
        function_tool(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "project_name": { "type": "string", "description": "目标项目名称。只有拿不到 project_id 时再使用。" },
                    "parent_id": { "type": "integer", "description": "父目录节点 ID。优先使用；不传表示在项目根目录创建。若创建根目录节点，不要传 null、空字符串、0或其他占位值。" },
                    "parent_path": { "type": "string", "description": "父目录路径，例如 产品文档/接口。只有拿不到 parent_id 时再使用，适合已知层级路径时定位。若没有父目录，就完全不要传这个字段。" },
                    "parent_name": { "type": "string", "description": "父目录名称。只有拿不到 parent_id 和 parent_path 时再使用，可能存在重名风险。若没有父目录，就完全不要传这个字段。" },
        ...
        function_tool(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "target_parent_id": { "type": "integer", "description": "目标父目录节点 ID。优先使用；仅在目标父目录真实存在时传入。若移动到项目根目录，不要传这个字段。" },
                    "target_parent_path": { "type": "string", "description": "目标父目录路径。只有拿不到 target_parent_id 时再使用。若移动到项目根目录，不要传这个字段。" },
                    "target_parent_name": { "type": "string", "description": "目标父目录名称。只有拿不到 target_parent_id 和 target_parent_path 时再使用。若移动到项目根目录，不要传这个字段。" },
                    "to_root": { "type": "boolean", "description": "是否直接移动到项目根目录。要移动到根目录时必须传 true；true 时不要再传任何 target_parent_*。" },
...
fn build_loop_system_prompt(
    ...
    let mut lines = vec![
        ...
        "当你要创建项目，或创建/编辑目录、文档时，应先通过工具读取当前项目列表、项目树、目标节点或编辑器/文档状态，再执行创建、移动、改写或保存；不要在不了解当前结构时直接猜测路径、父目录或文档状态。".to_string(),
    ...
    M::StreamingResponse: Clone + Unpin + rig::completion::GetTokenUsage,
{
    ...
    let mut tools = agent_function_tools();
    tools.extend(mcp_registry.definitions().iter().cloned());
        ...
        while let Some(item) = stream.next().await {
            match item {
                ...
                Ok(StreamedAssistantContent::ToolCall { tool_call, .. }) => {
                    ...
                    let output = if let Some(result) = mcp_registry
                        .execute(tool_call.function.name.as_str(), &arguments)
                        .await
                    {
                        let _ = send_json_event(
                            tx,
                        ...
                        match request_frontend_tool_output(
                            tx,
                            &run_id,
                            &PendingFrontendToolCall {
                                id: tool_call.id.clone(),
...
pub async fn chat_stream(
    ...
    let model_tools_enabled = model_config
        .as_ref()
        .and_then(|config| config.tools_enabled)
        .unwrap_or(true);
    ...
    tokio::spawn(async move {
        let mcp_registry = if model_tools_enabled {
            match load_mcp_settings(&db, user_id).await {
                Ok(Some(settings)) if settings.enabled == 1 => match list_mcp_servers(&db, user_id).await {
                    Ok(rows) => {
                        let enabled_rows = rows
                            .into_iter()
                            .filter(|row| row.enabled == 1)
                            .collect::<Vec<_>>();
                        let build_result = mcp_chat::build_mcp_chat_registry(
                            &mcp_manager,
                            &runtime_config,
                            &enabled_rows,
                        )
                        ...
                        for warning in &build_result.warnings {
                            let _ = send_json_event(
                                &tx,
                                "warning_event",
                                json!({ "scope": "mcp", "message": warning }),
                            )
                            .await;
                        }
            ...
            if matches!(load_mcp_settings(&db, user_id).await, Ok(Some(settings)) if settings.enabled == 1) {
                let _ = send_json_event(
                    &tx,
                    "warning_event",
