<template>
  <div class="doc-tree">
    <div class="tree-header">
      <span class="tree-label">我的文档</span>
      <div class="tree-actions">
        ...
        <button class="icon-btn" title="刷新" @click="docs.fetchTree()">
...
function ctxAction(action: 'open' | 'rename' | 'share' | 'delete' | 'new-doc' | 'new-dir') {
...
async function confirmRename() {
  ...
  try {
    await docs.updateNode(renameTarget.value.id, { name: renameValue.value.trim() })
...
const TreeNode = defineComponent({
  ...
  emits: ['select', 'toggle', 'create', 'rename', 'delete', 'share', 'contextmenu'],
  setup(props, { emit }) {
    ...
    return () => {
      ...
      const rowEl = h(
        'div',
        {
          class: ['tree-node-row', { 'is-selected': isSelected, 'is-dir': isDir.value }],
